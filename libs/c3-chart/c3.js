            ...
            eventRectEvent.call(
                config.data_selection_draggable && $$.drag ? (
                    d3.behavior.drag().origin(Object)
                        .on('drag', function () { $$.multiselectdrag(d3.mouse(this)); })
                        .on('dragstart', function () { $$.multiselectdragstart(d3.mouse(this)); })
                        .on('dragend', function () { $$.multiselectdragend(d3.mouse(this)); })
                ) : function () {}
            );
        } else {
            eventRectEvent.call(
...
/////////////////////////////multimulti//////////////////////////////////////
    ...
    c3_chart_internal_fn.multiselectdrag = function (mouse) {
        ...
        $$.randomUid = $$.ran;
        var blockElement = $$.main.select('.' + CLASS.dragarea+$$.randomUid);
        if (!blockElement[0][0]) {
            $$.main.select('.' + CLASS.chart).append('rect')
                            .attr('class', CLASS.dragarea+$$.randomUid)
                            .style('opacity', 0.3);
        }
        ...
        main.selectAll('.' + CLASS.shapes).selectAll('.' + CLASS.shape)
            ...
            .each(function (d, i) {
                ...
                if (isWithin ^ isIncluded) {
                    $$.multiselectedData.push(shape);
                    shape.classed(CLASS.INCLUDED, !isIncluded);
                    // TODO: included/unincluded callback here
                    shape.classed(CLASS.SELECTED, !isSelected);
                    // toggle.call($$, !isSelected, shape, d, i);
                    $$.selectPoint(shape, shape[0][0].__data__, shape[0][0].__data__.index)
                }
            });
    };
    ...
    c3_chart_internal_fn.multiselectdragstart = function (mouse) {
        ...
        if (dragArea[0].length + 1 <= $$.config.multiselectionCount) {
           var ran = Math.round(Math.random() * 120);
            $$.ran = ran;
            $$.dragging = true;
        } else {
            $$.dragging = false;
    ...
    c3_chart_internal_fn.multiselectdragend = function (mouse) {
        var $$ = this, config = $$.config;
